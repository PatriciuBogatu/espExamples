...
esp_err_t bsp_i2c_init_another(i2c_port_t i2c_num, uint32_t clk_speed)
{
...
static esp_err_t bsp_i2s_deinit(i2s_port_t i2s_num)
{
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    // Handle both TX and RX channels for I2S_NUM_1
    if (i2s_num == I2S_NUM_1)
    {
        if (tx_handle)
        {
            ESP_LOGI("BspBoard", "Cleaning TX");
        ...
        if (rx_handle)
        {
            ESP_LOGI("BspBoard", "Cleaning RX");
            ret_val |= i2s_channel_disable(rx_handle);
            ESP_LOGI("BspBoard", "Cleaning RX Channel disabled");
    ...
    {
        // Handle other I2S ports if needed
    }
#else
...
void bsp_board_deinit()
{
    ESP_LOGI("BspBoard", "Deiniting codecs");
    bsp_codec_deinit();
    vTaskDelay(pdMS_TO_TICKS(100));
    ESP_LOGI("BspBoard", "Deiniting I2S");
    bsp_i2s_deinit(I2S_NUM_1);
}
